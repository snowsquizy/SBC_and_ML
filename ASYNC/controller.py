...
# Removes Incorrect CPU type warning and others
...
"""
# If using cluster setup hardware address
if use_cluster:
    # Parameter Server IP Address and Port Number
    # Tensorflow accepts any unassigned port number 
    p_server = [
        "localhost:2222",
        "localhost:2223"]
    ...
    if (len(sys.argv) < 2):
        ...
        FLAGS.task_index = 0
    # convert FLAGS data to single variable
    node = []
    node.append(FLAGS.job_name)
    node.append(FLAGS.task_index)
    # tf object defining all hardware
    cluster = tf.train.ClusterSpec(
        {"ps": p_server, "worker": workers})
    # tf distributed hardware setup
    server = tf.train.Server(
        cluster,
        job_name=node[0],
        task_index=node[1])
    # If parameter server no other information is required to start it
    if node[0] == "ps":
        if debug_code:
            print("*** Parameter Server ***")
        server.join()
# When not using cluster assign dummy variables
else:
    p_server = []
    workers = []
    cluster = []
...
"""
Main Program Block for Neural Network Trainer
"""
"""
Check / Create Directory for Files Created
"""
...
print(
    ...
    str(N_N.start_time) + "," +
    str(N_N.end_time) + "," +
    str(N_N.end_time - N_N.start_time) + "," +
    str(N_N.final_accu))
